...
/*   Updated: 2023/01/18 00:11:15 by jahlee           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_gnl_list	*find_fd(t_gnl_list *tmp, int fd, int read_cnt)
{
	...
	while (tmp)
	{
		if (tmp->fd_idx == fd)
			break ;
		head = tmp;//수정됨!!(while밖에서 가져옴)
		tmp = tmp->next;
	}
	if (tmp)// 밑에 else if랑 묶어도 될듯??
		return (tmp);
	else if (read_cnt > 0)//=> (read_cnt > 0 && !tmp)
	{
		...
		// tmp->previous = NULL;//새로 만들었을때 뒤에 이어주는 것도 좀 수정해야할듯
		// tmp->backup = NULL;
		tmp->eof = 0;
		tmp->fd_idx = fd;
		// tmp->next = NULL;
